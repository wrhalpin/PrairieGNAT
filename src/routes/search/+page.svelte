<script lang="ts">
  ...
  function handleSearch(e: Event) {
    ...
    query = input.value;
    // TODO: implement search across all bundles
  }
</script>

<svelte:head>
  <title>Search — GNAT Reader</title>
</svelte:head>
...
<div class="p-4 pb-20">
  ...
  <input
    type="text"
    placeholder="Search all bundles..."
    value={query}
    ...
    <div class="space-y-2">
      {#each results as result}
        ...
        >
          ...
          <p class="text-sm text-slate-600 dark:text-slate-400">{result.type}</p>
        </a>
      {/each}
    </div>
  {/if}
