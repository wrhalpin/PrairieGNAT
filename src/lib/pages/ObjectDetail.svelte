<script lang="ts">
  ...
  function buildRelated(
    ...
    const relationshipIds =
      (p.relationshipsBy as Map<string, string[]>).get(oId) || [];
    const entries: RelatedEntry[] = [];
    const seen = new Set<string>();

    for (const relId of relationshipIds) {
      const rel = byId.get(relId) as
        | (StixObject & Record<string, unknown>)
        | undefined;
      if (!rel) continue;
      ...
      if (rel.type === 'relationship') {
        ...
        const otherId = (outgoing ? rel.target_ref : rel.source_ref) as
          | string
          | undefined;
        ...
        entries.push({
          obj: other,
          label: outgoing ? `${relType} →` : `← ${relType}`,
        });
  ...
  function getTLPLabel(markingIds: string[] | undefined): string {
    ...
    for (const markingId of markingIds) {
      ...
      const tlp = (marking?.definition as Record<string, unknown> | undefined)
        ?.tlp;
  ...
  function openObject(relObjectId: string) {
    dispatch('navigate', {
      route: 'object',
      params: { bundleId, objectId: relObjectId },
    });
  }
</script>
...
<div class="p-4 pb-20">
  ...
  {:else if obj}
    <div
      class={`p-4 rounded-lg mb-4 ${getTLPBgColor(obj.object_marking_refs)}`}
    >
      <div class="flex justify-between items-start mb-2">
        <h1 class="text-2xl font-bold">
          {getObjectIcon(obj.type)}
          {obj.name || obj.id.split('--')[1]}
        </h1>
        {#if obj.object_marking_refs}
          <span
            class="px-2 py-1 rounded text-xs font-semibold bg-slate-200 dark:bg-slate-700"
          >
    ...
    {#each getObjectFields(obj) as field}
      <div class="mb-3 p-3 rounded-lg bg-slate-100 dark:bg-slate-800">
        ...
        {#if field.type === 'code'}
          <p
            class="text-xs font-mono bg-slate-900 dark:bg-slate-700 text-green-400 p-2 rounded overflow-auto"
          >
            {field.value}
          </p>
        {:else if field.type === 'chip'}
          <div class="flex flex-wrap gap-1">
            {#each Array.isArray(field.value) ? field.value : [field.value] as item}
              <span
                class="px-2 py-1 rounded-full text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200"
              >
                {item}
              </span>
            {/each}
      ...
      <div class="mb-4">
        ...
        <div class="flex flex-wrap gap-1">
          {#each obj.labels as label}
            <span
              class="px-2 py-1 rounded text-xs bg-slate-200 dark:bg-slate-700"
            >
              {label}
            </span>
          {/each}
          ...
          {#each relatedObjects as related (related.obj.id + related.label)}
            ...
            >
              <p class="font-semibold text-sm">
                {getObjectIcon(related.obj.type)}
                {related.obj.name || related.obj.id.split('--')[1]}
              </p>
