<script lang="ts">
  ...
  function searchBundles(searchQuery: string) {
    ...
    outer: for (
      ...
      for (const obj of (
        parsed.objectsById as Map<string, StixObject>
      ).values()) {
        ...
        if (
          ...
          const description = (obj as Record<string, unknown>).description as
            | string
            | undefined;
...
<div class="p-4 pb-20">
  ...
  {#if results.length === 0}
    <p class="text-center text-slate-500 dark:text-slate-400 py-8">
      {query
        ? 'No matches found'
        : 'Enter a search query to find objects across all loaded bundles'}
    </p>
  {:else}
    <div class="mb-4">
      <p class="text-sm text-slate-600 dark:text-slate-400 mb-3 font-semibold">
        Found {filteredResults.length} result{filteredResults.length === 1
          ? ''
          : 's'}
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-1">
            ...
            {#each uniqueBundles as bundleId}
              <button
                on:click={() =>
                  (filterBundle = filterBundle === bundleId ? null : bundleId)}
                class={`px-2 py-1 rounded text-xs font-semibold transition ${
            ...
            <div class="flex justify-between items-start">
              <div class="flex-1">
                <p class="font-semibold text-sm">
                  {getObjectIcon(result.obj.type)}
                  {getObjectName(result.obj)}
                </p>
                ...
                {#if result.snippet}
                  <p
                    class="text-xs text-slate-600 dark:text-slate-400 mt-1 italic"
                  >
                    "{result.snippet}"
                  </p>
                {/if}
              </div>
              <p class="text-xs text-slate-500 ml-2 flex-shrink-0">
                {bundles[result.bundleIndex].name}
              </p>
            </div>
          </button>
        {/each}
